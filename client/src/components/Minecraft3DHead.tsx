...
export const Minecraft3DHead: React.FC<Minecraft3DHeadProps> = ({
  ...
  const half = size / 2;
  const faceClass = "absolute inset-0 rounded-sm border border-emerald-500/20 overflow-hidden bg-zinc-800";
  ...
  return (
    <div className={`flex min-w-0 max-w-full flex-col items-center ${className}`}>
      ...
      >
        ...
        >
          <div className="absolute inset-0 rounded-sm border border-emerald-500/30 overflow-hidden bg-zinc-900 shadow-lg" style={{ transform: `translateZ(${half}px)`, imageRendering: "pixelated" }}>
          ...
          <div className="absolute inset-0 rounded-sm border border-emerald-500/20 overflow-hidden bg-zinc-900" style={{ ...faceStyle, transform: `rotateY(180deg) translateZ(${half}px)`, filter: "brightness(0.65)" }}>
            <img src={faceImg} alt="" className="h-full w-full object-cover" onError={handleTextureError} />
          </div>
        </div>
        <div className="pointer-events-none absolute -bottom-3 left-1/2 h-3 w-3/4 -translate-x-1/2 rounded-full bg-emerald-500/20 blur-sm transition-all duration-200" style={{ transform: `scale(${isHovered ? 1.15 : 1})`, opacity: isHovered ? 0.7 : 0.4 }} />
      </div>
      ...
      {showProfileLink && (
        <div className="mt-3 max-w-full text-center">
          <a
            ...
            className="group inline-flex max-w-full items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-950/40 px-2.5 py-1 text-sm font-semibold text-emerald-400 transition-colors hover:bg-emerald-900/50 hover:text-emerald-300"
          >
